...
describe('[GET] /shelfs (E2E)', () => {
  ...
  it('should paginate shelfs and return correct meta data', async () => {
    ...
    for (let i = 0; i < totalShelfs; i++) {
      await shelfFactory.makePrismaShelf({
        companyId: company.id,
      });
    }

    const response = await request(app.getHttpServer())
      .get(`/shelfs?page=${page}&itemsPerPage=${itemsPerPage}`)
      .set('Cookie', `token=${accessToken}`);

    expect(response.statusCode).toBe(200);

    const { shelfs, meta } = response.body;

    expect(Array.isArray(shelfs)).toBe(true);
    expect(shelfs.length).toBe(itemsPerPage);
