...
import { FindShelfByIdParams, paramsValidationPipe } from './schemas/find-shelf-by-id-schema';
...
export class FindShelfByIdController {
  ...
  @Get(':id')
  @HttpCode(200)
  async handle(@Param(paramsValidationPipe) { id }: FindShelfByIdParams, @CurrentUser() user: UserPayload) {
    const result = await this._findShelfByIdUseCase.execute({ authenticateId: user.userId, shelfId: id });
