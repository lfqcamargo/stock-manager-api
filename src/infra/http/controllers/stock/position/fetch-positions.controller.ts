...
import { FetchPositionsQuery, queryValidationPipe } from './schemas/fetch-positions-schema';
...
export class FetchPositionsController {
  ...
  @Get()
  @HttpCode(200)
  async handle(@Query(queryValidationPipe) query: FetchPositionsQuery, @CurrentUser() user: UserPayload) {
    ...
    return { positions: positions?.map((item) => PositionPresenter.toHTTP(item)), meta };
  }
}
