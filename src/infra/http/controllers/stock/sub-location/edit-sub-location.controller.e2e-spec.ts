...
describe('[PUT] /sub-locations/:id (E2E)', () => {
  ...
  it('should edit a sub-location when user is admin or manager', async () => {
    ...
    const response = await request(app.getHttpServer())
      .put(`/sub-locations/${subLocation.id.toString()}`)
      .set('Cookie', `token=${accessToken}`)
      .send({
        code: 'SUB',
        name: 'Sub-Localização Editada',
        description: null,
      });
    ...
    expect(subLocationOnDatabase).toBeTruthy();
    expect(subLocationOnDatabase?.code).toEqual('SUB');
    expect(subLocationOnDatabase?.name).toEqual('Sub-Localização Editada');
    expect(subLocationOnDatabase?.description).toEqual(null);
  });
});
