...
describe('DeleteLocationUseCase', () => {
  ...
  it('should delete a location, its sub-locations, and addressings successfully', async () => {
    ...
    const addressing1 = makeAddressing({
      companyId: user.companyId,
      locationId: location.id,
      amount: 0,
    });
    const addressing2 = makeAddressing({
      companyId: user.companyId,
      locationId: location.id,
      amount: 0,
    });
    await addressingsRepository.create(addressing1);
    await addressingsRepository.create(addressing2);
    ...
    expect(locationsRepository.items).toHaveLength(0);
  ...
  it('should return AddressingHasBalanceError if location has addressings with balance', async () => {
    const user = makeUser({ role: UserRole.ADMIN });
    await usersRepository.create(user);
    ...
    const addressing = makeAddressing({
      companyId: user.companyId,
      locationId: location.id,
      amount: 10, // Ensure we have balance
    });
