...
export interface ShelfCsvRow {
  code: string;
  name: string;
  description?: string;
}

interface ImportShelfsCsvUseCaseRequest {
  authenticateId: string;
  mode: string;
  rows: ShelfCsvRow[];
}

type ImportShelfsCsvUseCaseResponse = Either<
  | UserNotFoundError
  | UserNotAllowedError
...
@Injectable()
export class ImportShelfsCsvUseCase {
  constructor(
  ...
  async execute({
    ...
    mode,
    rows,
  }: ImportShelfsCsvUseCaseRequest): Promise<ImportShelfsCsvUseCaseResponse> {
    ...
    if (importMode === ImportMode.RESET) {
      await this._shelfsRepository.deleteMany({ companyId });
    }
    ...
    for (const row of rows) {
      ...
      const existing = await this._shelfsRepository.findByCode(companyId, code);
      ...
      if (existing) {
        ...
        await this._shelfsRepository.update(existing);
      } else {
        await this._shelfsRepository.create(
          Shelf.create({
