...
export interface RowCsvRow {
  code: string;
  name: string;
  description?: string;
}

interface ImportRowsCsvUseCaseRequest {
  authenticateId: string;
  mode: string;
  rows: RowCsvRow[];
}

type ImportRowsCsvUseCaseResponse = Either<
  | UserNotFoundError
  | UserNotAllowedError
...
@Injectable()
export class ImportRowsCsvUseCase {
  constructor(
  ...
  async execute({
    ...
    mode,
    rows,
  }: ImportRowsCsvUseCaseRequest): Promise<ImportRowsCsvUseCaseResponse> {
    ...
    if (importMode === ImportMode.RESET) {
      await this._rowsRepository.deleteMany({ companyId });
    }
    ...
    for (const row of rows) {
      ...
      const existing = await this._rowsRepository.findByCode(companyId, code);
      ...
      if (existing) {
        ...
        await this._rowsRepository.update(existing);
      } else {
